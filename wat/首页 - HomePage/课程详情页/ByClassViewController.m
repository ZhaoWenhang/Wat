...
//            }
            ...
            shijiMoney2.textColor = [UIColor redColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        addByClassUserInfoViewController *vc = [addByClassUserInfoViewController new];
        vc.navTitle = @"购买信息";
        vc.returnByClassUserModel = ^(ByClassUserModel *byClassUserModel) {
            self.byClassUserModel = byClassUserModel;
            
            NSLog(@"%@%@%@%@",self.byClassUserModel.name,self.byClassUserModel.telephone,self,byClassUserModel.company,self.byClassUserModel.job);
