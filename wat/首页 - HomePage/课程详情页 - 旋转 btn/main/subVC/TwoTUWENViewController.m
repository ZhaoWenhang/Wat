...
@interface TwoTUWENViewController ()<WKScriptMessageHandler,WKNavigationDelegate,WKUIDelegate>
@property (nonatomic, strong)WKWebView *webView;
@property (nonatomic, strong) UIProgressView *progressView;
@end

@implementation TwoTUWENViewController
...
- (void)viewDidLoad {
    ...
    self.rightTitles = @[@"分享"];
    self.navTitle = self.watHomeClassListdetailModel.title;
    [self MKWebViewre];
...
- (void)MKWebViewre {
    ...
    // 3.创建Request
    NSURLRequest *request =[NSURLRequest requestWithURL:[NSURL URLWithString:self.watHomeClassListdetailModel.url]];
    // 4.加载网页
...
- (void)rightBtnsAction:(UIButton *)button{
    ...
    shareModel.title = self.watHomeClassListdetailModel.title;
    shareModel.descr = self.watHomeClassListdetailModel.subTitle;
    shareModel.url = [NSString stringWithFormat:@"%@",self.watHomeClassListdetailModel.share_url];
    UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:self.watHomeClassListdetailModel.thumb]];
