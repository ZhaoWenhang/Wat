...
- (void)panBack:(CADisplayLink *)link {//第一次滑动
    NSLog(@"44444444");
        ...
        [UIView animateWithDuration:0.5 animations:^{
            ...
            if (self.showFloatBall) {        
                ...
                if ([self haveIconImage]) {
                    ...
                    NSURL *imgUrl = [self.floatViewController valueForKey:@"hk_iconImage"];
                    [self.floatBall.iconImageView sd_setImageWithURL:imgUrl];
                }
...
- (void)floatBallDidClick:(HKFloatBall *)floatBall{ //点击球, 进入界面
...
-(void)floatBallEndMove:(HKFloatBall *)floatBall{ //移除浮球
...
- (void)setShowFloatBall:(BOOL)showFloatBall{//显示浮球 01
    NSLog(@"111111111111");
...
-(HKFloatAreaView *)cancelFloatArea{ //移动浮球 显示浮窗
    if (!_cancelFloatArea) {
