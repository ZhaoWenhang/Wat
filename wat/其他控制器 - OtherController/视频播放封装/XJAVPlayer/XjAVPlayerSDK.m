...
- (void)pausePlay{
    [self.xjPlayer xjPause];
}
- (CGFloat)xjCurrentTime{
    return self.bottomMenu.xjCurrentTime;//self.xjPlayer.xjCurrentTime; //wh 2018.7.9 修改
}
- (CGFloat)xjTotalTime{
    return self.bottomMenu.xjTotalTime;  //self.xjPlayer.xjTotalTime; //wh 2018.7.9 修改
}
...
- (void)xjAVPlayerBLock{
            ...
            switch (status) {
                    ...
                    //NSLog(@"GPRS网络");
                {
                    ...
                    [[ZWHHelper sharedHelper]addAlertViewControllerToView:[ZWHHelper rootTabbarViewController] withClickBlock:^(int clickInt) {
                        if (clickInt == 0) {
                           [[NSNotificationCenter  defaultCenter]postNotificationName:WatVideoNotPlayPopControllerNotification object:nil];
                        }else{
                           weakSelf.bottomMenu.xjPlay = YES;//如果想一进来就播放，就放开注释
                        }
                    } title:@"提示" content:@"当前为移动网络,是否继续观看?" cancleStr:@"稍后再看" confirmStr:@"继续观看"];
                }
                    break;
                case 2:
                    //NSLog(@"wifi网络");
    ...
    self.xjPlayer.xjCurrentTimeBlock = ^(CGFloat time){
        ...
        NSLog(@"%lf",weakSelf.bottomMenu.xjCurrentTime);
    };
    //播放完
    ...
    self.backView.userTapGestureBlock = ^(NSUInteger number,BOOL flag){
        if (number == 1) {
            [UIView animateWithDuration:0.3 animations:^{
                ...
                [[UIApplication sharedApplication] setStatusBarHidden:flag];//隐藏状态栏
            }];
        }else if (number == 2){
...
- (XJTopMenu *)topMenu{
    if (_topMenu == nil) {
        ...
        _topMenu.backgroundColor = [UIColor colorWithRed:50.0/255.0 green:50.0/255.0 blue:50.0/255.0 alpha:0.3];//
        _topMenu.hidden = YES;
        [self xjTopMenuBlock];
    }
    return _topMenu;
...
- (XJBottomMenu *)bottomMenu{
    if (_bottomMenu == nil) {
        ...
        _bottomMenu.backgroundColor = [UIColor colorWithRed:50.0/255.0 green:50.0/255.0 blue:50.0/255.0 alpha:0.3];
...
- (void)layoutSubviews{
    ...
    self.topMenu.frame = CGRectMake(0, MyStatusBarHeight, self.backView.width, 40);
