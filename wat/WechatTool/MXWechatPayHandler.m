...
+ (void)jumpToWxPayBackDic:(WeChatPayGetDetailModel *)weChatPayGetDetailModel
{
    //============================================================
    // 支付流程实现
    // 客户端操作     (实际操作应由服务端操作)
...
//
             PayReq *request = [[PayReq alloc] init];
             request.openID = weChatPayGetDetailModel.appid;
             request.partnerId = weChatPayGetDetailModel.partnerid;
             request.prepayId = weChatPayGetDetailModel.prepayid;
             request.package = weChatPayGetDetailModel.package;
             request.nonceStr = weChatPayGetDetailModel.noncestr;
             request.timeStamp = weChatPayGetDetailModel.timestamp;
             //request.sign = weChatPayGetDetailModel.sign;
